// Round 2: 5 example + 1 query，使用新抓的 5 篇 baseline
// Query 故意选短句（24 chars）跟 Round 1 同长度，方便对照
...
const baselinePath = "/Users/mahaoxuan/Desktop/黑客松/眺览二次开发/experiments/baseline-capture/outputs/2026-06-13T13-12-04-284Z-tillglance-baseline-5new.json";
const baseline = JSON.parse(fs.readFileSync(baselinePath, "utf8"));

const queryText = "深秋的街道铺满落叶，远处传来咖啡店轻柔的音乐。";

// 先抓这一段 query 在 TillGlance 的 baseline 起点
...
function buildPrompt({ numExamples, withLengthConstraint }) {
  ...
  const pids = Object.keys(baseline.contentDict).sort();
  const picks = pids.slice(0, numExamples);
  for (const pid of picks) {
    lines.push(`Paragraph ${pid}: ${baseline.contentDict[pid]}`);
...
async function callOnce({ numExamples, withLengthConstraint, label }) {
...
const r2a = await callOnce({ numExamples: 5, withLengthConstraint: false, label: "round2-5shot-no-length-rule" });
await new Promise((r) => setTimeout(r, 1500));
const r2b = await callOnce({ numExamples: 5, withLengthConstraint: true, label: "round2-5shot-with-length-rule" });

const outPath = "/Users/mahaoxuan/Desktop/黑客松/眺览二次开发/agent-swarm-output/agent-4-few-shot-iter/rounds/round-2.json";
fs.writeFileSync(outPath, JSON.stringify({
  ranAt: new Date().toISOString(),
  model,
  queryText,
  queryBaselineStarts: baselineQuery.starts,
  queryBaselineRaw: baselineQuery.raw,
  queryBaselineLatencyMs: baselineQuery.latencyMs,
  results: [r2a, r2b]
}, null, 2));
console.log(`\n# saved: ${outPath}`);
